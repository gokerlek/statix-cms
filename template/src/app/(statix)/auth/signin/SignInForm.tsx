...
export interface SignInFormProps {
  /** True when GITHUB_CLIENT_ID + GITHUB_CLIENT_SECRET are configured. */
...
/**
 * Sign-in flow. Better Auth's React client returns `{ data, error }` from
 * every call without throwing for auth/business errors, so we destructure
 * the result rather than wrapping in try/catch (catch never fires).
 *
 * Social-provider buttons render only when the corresponding OAuth env vars
 * are present (passed in by the server page.tsx wrapper). Without this
 * guard, clicking an unconfigured provider hits a 500.
 */
export function SignInForm({ githubEnabled, googleEnabled }: SignInFormProps) {
  ...
  async function handleSendOtp(e: React.FormEvent) {
    ...
    if (err) {
      const msg = err.message || t("signin.toasts.error.codeSendFailed");
      setError(msg);
      toast.error(msg);
      return;
    }
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    if (err) {
      setLoading(false);
      const msg = err.message || t("signin.toasts.error.invalidCode");
      setError(msg);
      toast.error(msg);
      return;
    }
  ...
  async function handleSocial(provider: "github" | "google") {
    ...
    if (err) {
      setLoading(false);
      const msg = err.message || t("signin.toasts.error.socialFailed");
      setError(msg);
      toast.error(msg);
    }
  }
